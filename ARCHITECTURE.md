...
* `src/lib/sendpulse/service.ts` — SendPulse REST API клиент с автоматическим кешированием Bearer-токенов, получением ботов (Telegram/Instagram), выгрузкой активных подписчиков, получением существующих цепочек/триггеров (`getProjectSendPulseFlows`) и управлением переменными (`setVariable`).
...
* `src/app/admin/LeadsDashboard/tabs/FunnelsTab.tsx` — Модуль сквозных воронок с поддержкой мульти-воронок, быстрым переключением между воронками проекта, импортом цепочек из SendPulse, адаптацией под органические запуски без платного трафика, конструктором вебхуков шагов и интерактивной панелью «Підписники бота & Сквозний маппінг (bw_cid)» с поиском, фильтрами и кнопкой синхронизации 1-в-1.
* `src/app/admin/LeadsDashboard/components/SendPulseFlowsModal.tsx` — Интерактивное модальное окно для прямого выбора, просмотра и импорта реальных цепочек (Flows) из SendPulse в качестве шагов воронки с сохранением оригинальных названий и мгновенной фильтрацией аудитории по участникам цепочки.
