...
* `src/app/admin/LeadsDashboard.tsx` — Главный интерфейс CRM для проектов экспертов. Поддерживает вкладки:
  ...
  6. **📋 Анкети** — інтерактивна панель (Master-Detail) для детального перегляду відповідей на анкетні форми та опросники по кожному респонденту. Дозволяє налаштовувати список анкетних лендінгів прямо в налаштуваннях проекту (`survey_landing_paths`), фільтрувати респондентів за вибраним лендінгом, здійснювати швидкий пошук (за ім'ям, телефоном, Telegram, Instagram та текстом відповідей), переглядати картку контактів з клікабельними посиланнями та кнопкою «🔍 Повний шлях клієнта» (`LeadJourneyModal`). Поля анкет витягуються динамічно з `raw_payload`, `metadata` та `diagnostics_comment` з інтелектуальним маппінгом українських міток та іконок.
