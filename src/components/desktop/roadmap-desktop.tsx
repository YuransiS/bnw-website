    ...
    <section ref={containerRef} className="relative py-32 bg-black text-white">
      <div className="container mx-auto px-6 max-w-5xl">
        ...
        <div className="roadmap-list relative max-w-4xl mx-auto">
          ...
          {/* Animated Fill Line */}
          <div ref={lineRef} className="absolute left-1/2 top-0 w-1 bg-white -translate-x-1/2 rounded-full h-0 shadow-[0_0_15px_rgba(255,255,255,0.5)]" />
          ...
          {roadmap.map((step, idx) => {
            ...
            return (
              <div
                ...
                className={`relative flex items-center mb-16 last:mb-0 ${isEven ? 'flex-row' : 'flex-row-reverse'}`}
              >
                {/* Dot */}
                <div className="absolute left-1/2 w-4 h-4 rounded-full bg-neutral-800 border-2 border-white/20 -translate-x-1/2 z-10 roadmap-dot transition-colors duration-300" />
                
                {/* Content */}
                <div className={`w-1/2 ${isEven ? 'pr-16 text-right' : 'pl-16 text-left'} roadmap-content`}>
