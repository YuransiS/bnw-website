      ...
      <div className="relative w-full max-w-xl h-full bg-[#0C0C0F] border-l border-white/10 p-6 shadow-2xl flex flex-col space-y-6 animate-in slide-in-from-right duration-300 z-10 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 gap-6 flex-grow overflow-hidden">
          ...
          >
            ...
            {history.length > 0 && (() => {
              const getTouchLandingDisplay = (touch: any) => {
                if (!touch) return "Головна сторінка";
                const raw = touch.metadata?.raw_row || {};
                const meta = touch.metadata || {};
                const path = touch.page_path || meta.page_path || meta.path || raw.page_path || "";
                const fullUrl = touch.page_url || meta.page_url || meta.full_url || raw.page_url || "";
                const sheet = touch.target_sheet || meta.target_sheet || meta.original_sheet || raw.target_sheet || raw.original_sheet || "";
                const tariff = meta.tariffName || meta.tariff_name || meta.tariff || raw.tariffName || raw.tariff || raw.raw_payload?.tariffName || meta.offer_title || "";

                if (path && path !== "/" && path !== "") {
                  if (path === "/minicourse") return "Міні-курс (/minicourse)";
                  if (path === "/web") return "Вебінар (/web)";
                  if (path === "/intensive") return "Інтенсив (/intensive)";
                  if (path === "/checkout") return "Оплата (/checkout)";
                  if (path === "/direct-payment") return "Пряма оплата (/direct-payment)";
                  if (path === "/diagnostics" || path === "/rozbir") return "Діагностика (/diagnostics)";
                  return path;
                }

                if (sheet && sheet !== "all" && sheet !== "") {
                  return sheet;
                }
                ...
                if (fullUrl) {
                  try {
                    ...
                    return `${u.hostname}/ (Головна)`;
                  } catch {
                    return fullUrl;
                  }
                }

                return "Головний лендинг (/)";
              };
                  ...
                  <div className="grid grid-cols-2 gap-3 text-[11px]">
                    <div className="space-y-1 bg-white/[0.02] p-2.5 rounded-xl border border-white/5">
                      ...
                      <p className="text-white font-bold truncate">
                        {firstTouch?.utm_source ? `UTM: ${firstTouch.utm_source}` : "Прямий перехід"}
                      </p>
                      <p className="text-white/70 text-[10px] truncate">
                        <span className="text-white/40">Лендинг: </span>
                        <span className="font-semibold text-emerald-400/90">{getTouchLandingDisplay(firstTouch)}</span>
                      </p>
                      {firstOffer && (
                          ...
                          <div className="flex flex-wrap gap-2">
                            {fullUrl && (
                              ...
                              >
                                ...
                                <span className="truncate max-w-[200px]" title={fullUrl}>
                                  {pathVal && pathVal !== "/" ? `Сторінка: ${pathVal}` : displayUrl || "Головний лендинг (/)"}
                                </span>
                            ...
                            {!fullUrl && pathVal && (
                              <span className="px-2.5 py-1 rounded-xl bg-blue-500/10 border border-blue-500/20 text-[10px] font-extrabold text-blue-450 flex items-center gap-1.5">
                                <Globe className="w-3 h-3 shrink-0 text-blue-400" />
                                {pathVal === "/" ? "Головний лендинг (/)" : `Сторінка: ${pathVal}`}
                              </span>
                            )}
