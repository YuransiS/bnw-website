...
export default async function AdminUsersPage() {
  ...
  // 3. Strict gate check - Only Founder / Developer / Superman allowed in Access Control
  if (profile?.role !== "founder" && profile?.role !== "developer" && profile?.role !== "superman" && profile?.role !== "admin") {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-6 font-sans">
        ...
        <Link
          href="/admin"
          className="px-6 py-3 rounded-full bg-white text-black font-extrabold text-sm hover:bg-neutral-200 transition-all shadow-[0_0_20px_rgba(255,255,255,0.15)] cursor-pointer"
        >
          Повернутися в CRM
        </Link>
      </div>
